  ...
  try{
    ...
    export function SoundProvider({children}:{children:React.ReactNode}){
      const [muted, setMuted] = useState<boolean>(()=>{
        try{ const v = localStorage.getItem('mz_sound_muted'); return v === '1' }catch(e){ return false }
      })
      // persist
      React.useEffect(()=>{ try{ localStorage.setItem('mz_sound_muted', muted ? '1' : '0') }catch(e){} },[muted])
      return <SoundContext.Provider value={{muted, setMuted}}>{children}</SoundContext.Provider>
    }
